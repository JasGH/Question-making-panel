import fontMetrics from 'src/persian-katex-plugin/src/fontMetrics.json' assert {type: 'json'};
const fontName = 'Vazir'

export default function PersianPlugin(katex, options) {
  const mainOptions = { fontName, fontMetrics }
  Object.assign(mainOptions, options)

  for (const fontName in mainOptions.fontMetrics) {
    const metrics = mainOptions.fontMetrics[fontName]
